// Client-only memory of the selected event category. Category selection has no
// URL, so we combine two mechanisms:
...
// The category is stored on the pushed entry (history.state.kdCat), which is
// authoritative for Back/Forward; sessionStorage is the fallback for forward
// navigations (breadcrumb Links, search, direct links).
...
// Home-page category pick: push a same-URL history entry so Back returns to the
// 4 categories, and remember it for forward navigations back to "/".
...
// Forget the remembered category (no history change). Used by the "category"
// breadcrumb crumb on event pages so it lands on the 4 categories, not the
// remembered event list.
